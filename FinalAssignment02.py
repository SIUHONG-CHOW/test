...
tesla_data = tic.history(period="max")
...
get_ipython().system('mamba install bs4==4.11.1 -y')
...
#tesla_revenue=pd.read_html(str(soup.find_all("table")[1]),header =0, flavor = 'bs4')
#df=pd.DataFrame(tesla_revenue,columns=['Date','Revenue'])
#tesla_revenue.columns=['Date','Revenue']
#print(tesla_revenue)
#print(str(soup.find_all("table")[1]))
#rows = []
...
#print(type(soup.find_all('table')[1]))
...
for row in table.tbody.find_all('tr'):    
    ...
    list_row=[date,revenue]
    #tesla_revenue=pd.concat([tesla_revenue,df], ignore_index=True)
    tesla_revenue.loc[len(tesla_revenue)] = list_row
...
gme_data = tic.history(period="max")
...
for row in table.tbody.find_all('tr'):    
    ...
    list_row=[date,revenue]
    #tesla_revenue=pd.concat([tesla_revenue,df], ignore_index=True)
    gme_revenue.loc[len(gme_revenue)] = list_row
